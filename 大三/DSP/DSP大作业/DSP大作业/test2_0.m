...
%读取文件
fid = fopen('C:\Users\惠普\Desktop\DSP大作业\DSP大作业\UEQ_rawFile_16bit_Task2.dat', 'r');
dataLen = inf;
...
L = 2^16;%给出FFT序列长度
q = 1.5;%判定互相关序列中一个值明显高于周围噪声的限度
w = 1.12;%判定弱信号存在的限度
b = 4;
c = 50000;
type = 0;%表示当前检测的一个卫星信号的强弱
cycnum = 7;
init1 = 0;%是否初始化了
init2 = 0;%是否初始化了
datafft = zeros(100,L);%存储输入信号的fft结果
...
for p = 1:63
    ...
    while (k+1)*(L-M+1)+M-1 <= cyc*cycnum
        ...
        if (flag1 == 0 && (k+1)*(L-M+1) >= 2*cyc)
            ...
            if (maxone >= q*compa && maxone >= q*compb)
                type = 1;%发现强卫星信号
            end
        end
    end
    init1 = 1;%初始化完毕，不用重复计算fft

    if (type == 1)%对于强信号，可以直接计算出其强度和延时信息
        ...
        D(p) = maxpos;
        sate = [sate,p];
    else%对于弱信号，通过分组求和，抵消噪声影响后，再次进行匹配
        relation = zeros(1,size(data,2)+2*M);
        k = 0;
        while (k+1)*(L-M+1)+M-1 <= cyc*cycnum
            if (init2 == 0)
